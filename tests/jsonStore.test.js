...
test('reads public content, starts with no personal state, and derives chapter status', async (t) => {
  ...
  const sentence = (await store.getSentence('daodejing', 'daodejing-01', 'daodejing-01-01')).sentence;
  assert.equal(sentence.original, '道可道，非常道。');
  assert.equal(sentence.userNote, '');

  await store.updateSentence('daodejing', 'daodejing-01', 'daodejing-01-01', { userNote: 'A question', status: 'uncertain' });
  assert.equal((await store.getChapter('daodejing', 'daodejing-01')).chapter.status, 'uncertain');
});

test('saving changes only the target notes record and never the public content files', async (t) => {
  const root = await fixture();
  t.after(() => fs.rm(root, { recursive: true, force: true }));
  const store = createJsonStore(root);
  ...
  await store.updateSentence('daodejing', 'daodejing-01', 'daodejing-01-01', { userNote: 'My reading', status: 'completed' });
  ...
  const merged = (await store.getSentence('daodejing', 'daodejing-01', 'daodejing-01-01')).sentence;
  assert.equal(merged.userNote, 'My reading');
});
...
test('serializes simultaneous saves for one work without losing either sentence', async (t) => {
  ...
  await Promise.all([
    store.updateSentence('daodejing', 'daodejing-03', 'daodejing-03-01', { userNote: 'First concurrent save', status: 'completed' }),
    store.updateSentence('daodejing', 'daodejing-03', 'daodejing-03-02', { userNote: 'Second concurrent save', status: 'uncertain' })
  ]);
...
test('rejects a note record whose original no longer matches its sentence', async (t) => {
  const root = await fixture();
  t.after(() => fs.rm(root, { recursive: true, force: true }));
  const store = createJsonStore(root);
  const notesPath = path.join(root, 'notes', 'daodejing.json');
  await store.updateSentence('daodejing', 'daodejing-01', 'daodejing-01-01', { userNote: 'Saved note', status: 'completed' });
  ...
  await assert.rejects(() => store.getWork('daodejing'), (error) => error instanceof StoreError && error.status === 500 && error.message.includes('integrity mismatch'));
});
...
test('missing work, chapter, and sentence return StoreError 404', async (t) => {
  ...
  for (const operation of [
    ...
    await assert.rejects(operation, (error) => error instanceof StoreError && error.status === 404);
  }
});
