function renderTextBasis(textBasis) {
  ...
  const limitations = textBasis.limitations || [];
  const layers = textBasis.editorialLayers || [];
  ...
  const layerText = layers.length
    ? `编辑层：${layers.map((item) => Reader.escapeHtml(item)).join('、')}`
    : '';

  return `
    <h2>文本依据</h2>
    <p>
      <strong>${Reader.escapeHtml(statusLabels[textBasis.status] || '状态未注明')}</strong>
      · ${Reader.escapeHtml(textBasis.baseText || '底本未注明')}
    </p>
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    const work = await Reader.getJson(`/api/works/${encodeURIComponent(workId)}`);
    ...
    document.querySelector('#work-meta').textContent = work.author || '传统归属未注明';
    textBasis.innerHTML = renderTextBasis(work.textBasis);
    textBasis.hidden = false;

    chapters.innerHTML = work.chapters.map((chapter) => `
      <a class="index-card chapter-card" href="/chapter.html?work=${encodeURIComponent(work.id)}&chapter=${encodeURIComponent(chapter.id)}">
        <div>
